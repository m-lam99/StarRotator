...
class StarRotator(object):
    ...
    def calculate_transit_times(self):
            ...
            transit[transit == 1] = 0
            transit[pos < 1 - self.Rp_Rs] = 1
            times = np.argwhere(transit == 1)

            return times

        
    def compute_unbroadened_spectra(self):
        ...
        A = self.Rp_Rs**2 * (
            (1 - self.u1 * (1 - mu) - self.u2 * (1 - mu) ** 2)
            / (1 - self.u1 / 3 - self.u2 / 6)
        )
        f_r = self.stellar_spectrum / A[:, np.newaxis] * (1 - self.residual)

        R = np.sqrt(self.xp**2 + self.yp**2)
        transit_idx = np.argwhere(R < 1 - self.Rp_Rs)
        r_outside = R + self.Rp_Rs
        r_inside = R - self.Rp_Rs
        area_t = np.pi * (r_outside**2 - r_inside**2)

        f_r = f_r[R < 1 - self.Rp_Rs][:]
        area_t = area_t[R < 1 - self.Rp_Rs]
        I = 1 - self.u1 * (1 - mu) - self.u2 * (1 - mu) ** 2
        I = I[R < 1 - self.Rp_Rs]
        R = R[R < 1 - self.Rp_Rs]

        R = R[~np.isnan(R)]
        area_t = area_t[~np.isnan(area_t)]
        I = I[~np.isnan(I)]

        return f_r, R, transit_idx
    ...
    def integrate_obs_spectrum(self):
        ...
        time_r_min = times[np.nanargmin(r_min)]
        mu_max = np.sqrt(1 - np.nanmin(r_min) ** 2)
        
        wl_base = self.wl
        f_r, R, transit_idx = self.compute_unbroadened_spectra()
        fr_interp = np.empty_like(f_r)
        v_list = np.empty_like(R)
        
        for i, idx in enumerate(transit_idx):
            from astropy import constants as C
            from scipy.interpolate import interp1d

            mask = self.masks[idx[0]].copy()
            mask[self.masks[idx[0]] == 1] = 0
            mask[np.isnan(self.masks[idx[0]])] = 1
            mask[mask == 0] = np.nan
            masked_vel_grid = mask * self.vel_grid
            ...
            fr_interp_temp = interp1d(wl, f_r[i, :], bounds_error=False)(wl_base)
            fr_interp[i] = fr_interp_temp
            
            
        R2_centre = fr_interp[np.nanargmin(r_min)]

        R2_result = R2_centre

        I0 = 1 - self.u1 * (1 - mu_max) - self.u2 * (1 - mu_max) ** 2
        R2_result *= A_centre * I0

        n = len(times)

        R2_temp = []
        # self.R2_temp.append(R2_centre)
        weight_sum = np.pi * np.min(r_min) ** 2
        pos = np.sqrt(self.xp**2 + self.yp**2)
        mus = np.sqrt(1 - pos**2)
        for i, t in enumerate(times):
            ...
            # Limb-darkening
            I = 1 - self.u1 * (1 - mus[t]) - self.u2 * (1 - mus[t]) ** 2
            R2 *= area_t * I
            R2_result += R2
            weight_sum += area_t * I

        # R2_result *= I0
        R2_result /= weight_sum
    ...
    def animate_complete(self):
        ...
        r_max[r_max > 1] = 1
        r_min[r_min > 1] = 1
        total_narrow_spectrum = self.integrate_obs_spectrum()/np.nanmax(self.integrate_obs_spectrum())
        fn,R, transit_idx = self.compute_unbroadened_spectra()
        max_flux = np.nanmax(fn)
        ...
        for i in range(self.Nexp):
            ...
            if i in times:
                ax[0][1].plot(self.wl, (fn[np.where(times == i)[0]]/np.nanmax(fn[np.where(times == i)[0]])).flatten(), color='rebeccapurple', lw=1)
